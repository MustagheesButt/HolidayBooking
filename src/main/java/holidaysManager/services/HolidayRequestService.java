...
public class HolidayRequestService {
  ...
  public List<HolidayRequest> findAllByEmployee(Employee employee) {
    try {
      return entityManager
        .createQuery("SELECT h FROM HolidayRequest h WHERE h.employee = ?1", HolidayRequest.class)
  ...
  public List<HolidayRequest> getAll() {
    return entityManager
      .createQuery("SELECT h FROM HolidayRequest h", HolidayRequest.class)
      .getResultList();
  }

  public List<HolidayRequest> getApproved() {
    return entityManager
      .createQuery("SELECT h FROM HolidayRequest h WHERE h.status = 'approved'", HolidayRequest.class)
      .getResultList();
  }

  public List<HolidayRequest> getPending() {
    return entityManager
      .createQuery("SELECT h FROM HolidayRequest h WHERE h.status = 'pending'", HolidayRequest.class)
      .getResultList();
  }
